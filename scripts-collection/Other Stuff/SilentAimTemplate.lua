...
local remote = game:GetService("ReplicatedStorage").Anchor.Remotes.PacketGateway
    ...
    newcclosure(
        function(self, ...)
            if typeof(self) == "Instance" then
                ...
                local remArgs = {...}
                if method and (method == "FireServer" and self == remote) and remArgs[1]["ID"] == "Fire" then
                                ...
                                if HumanoidRootPart then
                                    ...
                                    if isVisible then
                                        local distanceToMouse =
                                            (mousePos - Vector2.new(screenPosition.X, screenPosition.Y)).Magnitude
                    ...
                    if closestPlayer then
                        remArgs[1]["Data"]["Meta"]["MouseHitPosition"] = closestPlayer.Character.Head.Position
                        remArgs[1]["Data"]["ShotArray"][1][1]["Position"] = closestPlayer.Character.Head.Position
                        remArgs[1]["Data"]["ShotArray"][1][1]["Part"] = closestPlayer.Character.Head
                        print(closestPlayer)
...
if remote then
    ...
    oldNamecall = hookmetamethod(remote,"__namecall",newcclosure(function(self, ...)
        if typeof(self) == "Instance" then --manual args guard
            local method = getnamecallmethod()
            local remArgs = {...}
            if method and (method == "FireServer" and self == remote) and remArgs[1]["ID"] == "Fire" then
                ...
                if targetHumanoidRootPart and LPlayer and LPlayer.Character and LPlayer.Character:FindFirstChild("HumanoidRootPart") then
    
                    print("Target:", target)
                    remArgs[5][1]["dir"] = targetHead.Position
                    remArgs[1]["Data"]["Meta"]["MouseHitPosition"] = targetHead.Position
                    remArgs[1]["Data"]["ShotArray"][1][1]["Position"] = targetHead.Position
                    remArgs[1]["Data"]["ShotArray"][1][1]["Part"] = targetHead.Head
                    setnamecallmethod(method)
                    return oldNamecall(self, unpack(remArgs))
                else
                    setnamecallmethod(method)
                    warn("Hook failed: missing player")
                end
            end
        end
        return oldNamecall(self, ...)
    ...
    warn("Remote event not found in the player's character.")
end
