--- Experimental AC Client Sided Killer
local old;
old = hookmetamethod(game, "__namecall", newcclosure(function(self, ...)
    if getnamecallmethod() == "FireServer" and self.Name == "VastoVfx" then
        return nil
    end
    return old(self, ...)
end))
    ...
    for i, v in ipairs(Characters:GetChildren()) do
        if not v:FindFirstChild("Health") then
            ...
            if underscoreIndex then
                ...
                print(v.Name ..
                          "Mob Name just changed which means it's spawned gl with that lol")
            end
        end
    end
end
...
local mobs = {}
local NPCs = {
    "Nel Tu", "Bambietta", "Kisuke", "Rin", "V - Unequaled, Unrivaled", "1"
}
local SelectedPlayer

for _, v in next, Characters:GetChildren() do
    if not v:FindFirstChild("Health") then table.insert(mobs, v.Name) end
end
...
Tab:Toggle({
    ...
    Callback = function(state)
        if state then
            ...
            farmHeartbeatConnection = rs.Heartbeat:Connect(function()
                local Mobs = Characters:GetChildren()
                local nearestModel = nil
                local nearestDistance = math.huge
                for i = 1, #Mobs do
                    local v = Mobs[i]
                    if v.Name ~= "LostSoul" and not v:FindFirstChild("Health") and
                        v:FindFirstChild("Humanoid").Health ~= 0 then
                        local distance =
                            (v:GetPivot().Position -
                                lp.Character.PrimaryPart.Position).Magnitude
                ...
                if nearestModel then
                    repeat
                        task.wait()
                        moveto(nearestModel:GetPivot(), 70)
                    until nearestModel == nil or state
                end
            end)
        else
            if farmHeartbeatConnection then
...
Tab:Toggle({
    ...
    Callback = function(state23)
        if state23 then -- Fixed variable name (state2 instead of state)
            ...
            farmHeartbeatConnection23 = rs.Heartbeat:Connect(function()
                game:GetService("ReplicatedStorage").Remotes.ServerCombatHandler:FireServer(
                    "LightAttack")
            end)
        else
            if farmHeartbeatConnection23 then
            ...
            farmHeartbeatConnection231 =
                rs.Heartbeat:Connect(function()
                    game:GetService("ReplicatedStorage").Remotes
                        .ServerCombatHandler:FireServer("CriticalAttack")
                end)
        else
            if farmHeartbeatConnection231 then
                farmHeartbeatConnection231:Disconnect()
...
Tab:Toggle({
    ...
    Default = false,
    Callback = function(state22d) AutoParry = state22d end
})
                ...
                for _, player in pairs(Characters:GetChildren()) do
                    if player.Name ~= lp.Name then
                        if player and player:FindFirstChild("HumanoidRootPart") then
                            ...
                            if distance <= 8 then
                                local currentState =
                                    player:GetAttribute("CurrentState")
                                if currentState ~= "Idle" and currentState ~=
                                    "Sprinting" and currentState ~=
                                    "WeaponDrawn" and currentState ~=
                                    "Unconscious" then
                                    shouldBlock = true
                                    break
                                end
...
local Dropdown = Tab:Dropdown({
    ...
    Callback = function(PlayerSelected) SelectedPlayer = PlayerSelected end
})
local Button = Tab:Button({
    ...
    Callback = function()
        ...
        Dropdown:UpdateList({Items = PlayersTable, Replace = true})
    end
})
...
Tab:Toggle({
    ...
    Callback = function(state2)
        if state2 then -- Fixed variable name (state2 instead of state)
            ...
            farmHeartbeatConnection2 = rs.Heartbeat:Connect(function()
                for i, v in pairs(Characters:GetChildren()) do
                    if v.Name == SelectedPlayer then
                        repeat
                            task.wait()
                            moveto(v:FindFirstChild("HumanoidRootPart").CFrame,
                                   70)
                        until v:FindFirstChild("Humanoid").Health == 0 or
                            not state2 -- Changed from 'state2' to 'not state2' to exit the loop when the toggle is turned off
                    end
                end
            end)
        else
...
Tab2:Button({
    ...
    Callback = function() lp.Character.Humanoid.Health = 0 end
})
...
Tab2:Slider({
    ...
    Callback = function(val22) lp.Character.Humanoid.HipHeight = val22 end
})
...
local function esp(entity)
    ...
    ancestryChangedConnection = entity.AncestryChanged:Connect(function(_,
                                                                        parent)
        if not parent then disconnectConnections() end
    end)

    healthChangedConnection = humanoid.HealthChanged:Connect(function(health)
        if health <= 0 then disconnectConnections() end
    end)

    renderSteppedConnection = runService.RenderStepped:Connect(function()
        local hrpPosition, hrpOnScreen =
            camera:WorldToViewportPoint(humanoidRootPart.Position)
        if hrpOnScreen then
            local distance = math.floor((humanoidRootPart.Position -
                                            camera.CFrame.Position).Magnitude +
                                            0.5)
            textLabel.Position = Vector2.new(hrpPosition.X, hrpPosition.Y)
            textLabel.Text = string.format(
                                 "%s | Health: %d | Distance: %d studs",
                                 entity.Name, humanoid.Health, distance)
...
local function toggleESP()
    ...
    for _, entity in ipairs(entitiesFolder:GetChildren()) do
        if not entity:FindFirstChild("Health") then entityAdded(entity) end
    end
end

userInputService.InputBegan:Connect(function(input)
    if input.KeyCode == Enum.KeyCode.G then toggleESP() end
end)
...
local function entityAdded(entity)
    if entity:IsA("Model") and entity:FindFirstChild("Health") and entity.Name ~=
        lp.Name then esp(entity) end
    entity.ChildAdded:Connect(function(child)
        if child:IsA("Model") and child:FindFirstChild("Health") and child.Name ~=
            lp.Name then esp(child) end
    end)
end
...
userInputService.InputBegan:Connect(function(input)
    if input.KeyCode == Enum.KeyCode.H then toggleESP() end
end)
