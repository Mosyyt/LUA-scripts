...
local function TPReturner(placeId)
    ...
    --- The JSON payload returned by games.roblox.com
    local Site;
    if foundAnything == "" then
        Site = HttpService:JSONDecode(game:HttpGet(
                                          'https://games.roblox.com/v1/games/' ..
                                              placeId ..
                                              '/servers/0?sortOrder=2&excludeFullGames=true&limit=100'))
    else
        Site = HttpService:JSONDecode(game:HttpGet(
                                          'https://games.roblox.com/v1/games/' ..
                                              placeId ..
                                              '/servers/0?sortOrder=2&excludeFullGames=true&limit=100&cursor=' ..
                                              foundAnything))
    end

    local ID = ""
    if Site.nextPageCursor and Site.nextPageCursor ~= "null" and
        Site.nextPageCursor ~= nil then foundAnything = Site.nextPageCursor end
    local num = 0;
    for i, v in pairs(Site.data) do
        ...
        if tonumber(v.maxPlayers) > tonumber(v.playing) then
            ...
            if Possible == true then
                ...
                task.wait()
                pcall(function()
                    writefile("server-hop-temp.json",
