...
local hwid = tostring(LocalPlayer and LocalPlayer.UserId) or "unknown_hwid"
local service = "mosyyt"
local apiKey = "8bb3ef27832b90c301fad9ff6078614837ae190e3daeea507a1f321ebbd53739"
...
-- Game-Specific Scripts Database
local GAME_SCRIPTS = {
    [12355337193] = { -- Murder Mystery 2
        name = "Murder vs Sheriff",
        script = "loadstring(game:HttpGet('https://pandadevelopment.net/virtual/file/525f370457dd91ee'))()",
        verified = true
    },
    -- Add more games here as needed:
    -- [PlaceID] = {name = "Game Name", script = "script URL", verified = true/false}
}
...
pcall(function()
    ...
    supportedGame = GAME_SCRIPTS[game.PlaceId]
end)
...
statusText.Text = supportedGame and 'Detected supported game: '..supportedGame.name or 'Initializing...'
...
local function playLoadingAnimations()
    ...
    local loadingSteps = {
        ...
        {
            text = supportedGame and 'Preparing '..supportedGame.name..' script' or 'Verifying game compatibility...',
            percent = 40,
            duration = 0.7,
        },
        {
...
-- Key System with Game Script Execution
function createKeySystem()
    ...
    statusLabel.Text = supportedGame and '🕹️ Supported game detected: '..supportedGame.name or 'Status: Waiting for key...'
    ...
    getKeyBtn.MouseButton1Click:Connect(function()
        ...
        wait(0.5)
         ...
         local keyLink = 'https://pandadevelopment.net/getkey?service=mosyyt&hwid=12121'
   ...
   checkBtn.MouseButton1Click:Connect(function()
    ...
    local url = "https://pandadevelopment.net/v2_validation?key=" .. userKey .. "&service=" .. service .. "&hwid=" .. hwid
    ...
    if not decodeSuccess then
        ...
        if response.code == 'KEY_VALID' then
            ...
            -- Execute game-specific script if available
            if supportedGame then
                statusLabel.Text = '🎮 Loading '..supportedGame.name..' script...'
                local success, err = pcall(function()
                    loadstring(supportedGame.script)()
                end)
                
                if not success then
                    statusLabel.Text = '⚠️ Error loading game script: '..tostring(err)
                    wait(2)
                end
            end
            
            -- Load main hub script
            api.load_script()
            gui:Destroy()
